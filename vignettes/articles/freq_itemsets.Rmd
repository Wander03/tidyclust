...
predict_cluster(object = fi_fit, new_data= new_data) |> class()

tidyclust:::format_cluster(list(a = c(bob = 2), b = c(joe = 5
                                                      )))
...
result_list <- lapply(result_list, function(df) {
    df$prob <- ifelse(df$pred & df$prob >= .5, 1, df$prob)
    df
  })
```
